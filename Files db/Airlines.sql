...
CREATE VIEW viewVoli AS 
	SELECT l1.nomeCitta AS Partenza, l2.nomeCitta AS Arrivo, ap.nome AS A1, aa.nome AS A2, v1.oraP AS OraPartenza, v1.oraA AS OraArrivo, vi.giorno, vi.prezzoSeconda AS Prezzo, c.nome AS Compagnia
	FROM (Viaggi vi NATURAL JOIN Tratte t) JOIN Compagnie c ON (vi.idCompagniaEsec=c.idCompagnia),
		...
		(((Viaggi vi2 NATURAL JOIN DettagliViaggi dv)NATURAL JOIN Voli v1)NATURAL JOIN Tratte t3)
	WHERE t.da=t1.da AND t.a=t2.a AND t.da=t3.da AND t.a=t3.a AND vi.idViaggio=vi2.idViaggio;
